        ...
        try { 
             ...
             if($sens==1)
            {
                if($uniteadmin_code==$ua_signataire_code) //
                 {
                  if ($type_courrier=='Interne') 
                  {
                    $courrierCourant = Courrierinitie::where('code', '=', $courrier_code)->first();
                    $secretariat = Uniteadmin::where('secretaire_de', '=', $courrierCourant->destinataireInterne_code)->first();
                    //Génération du code
                    ...
                    $courrierinterne->objet=$courrierCourant->objet;
                    $courrierinterne->courrierinitiedepart_code=$courrierCourant->code;
                    $courrierinterne->expediteur_code=$uniteadmin_code;

                    $courrierinterne->recepteur_code=$courrierCourant->destinataireInterne_code;/*
                    $courrierinterne->uasecretaire_code =$ua_secretaire_code;*/
                    if ($secretariat) {
                      /*$courrierinterne->transmission =false;
                      $courrierinterne->referenceAttribuee='';*/
                      $courrierinterne->uasecretaire_code=$secretariat->code;
                    }
                    $courrierinterne->transmission =true;
                    $courrierinterne->referenceAttribuee=$courrierCourant->referenceCourrier;
                    $courrierinterne->typecourrier_code=$courrierCourant->typecourrier_code;
                    $courrierinterne->referenceCourrier=$courrierCourant->referenceCourrier;
                    $courrierinterne->ampliataires=$courrierCourant->ampliataires;
                    $courrierinterne->dateCourrier=$courrierCourant->dateCourrier;
                    $courrierinterne->dateReception=date("Y-m-d H:i:s");
                    $courrierinterne->resumeCourrier=$courrierCourant->texteCourrier;
                    $courrierinterne->codeCourrierReference='';
                    $courrierinterne->siReponseCourrier=0;
                    $courrierinterne->typeRequete=0;
                  ...
                  //Mettre à jour la table courrier initié
                  Courrierinitie::where("code","=",$courrier_code)->update(["finTraitement" =>1]);
                 }
                else
                {
                  //Initialisation du courrier dans la table parcours
