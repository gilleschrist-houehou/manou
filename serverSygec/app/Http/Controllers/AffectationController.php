...
//verifie les champs fournis
          if (!(  isset($inputArray['affectations'])
           ...
           foreach($affectations as $oc)
           {
              ...
              $check=Affectation::where('courrierinterne_code','=',$courrier_code)
                ->where("uniteadmin_code","=",$uniteadmin_code)
                ->first();
              ...
              if(empty($check))
              {
                ...
                ->where("uniteadmindirection_code","=",$uniteadmin_code)->first();
                  ...
                  {
                     //Génération du code
                    $code=MyfunctionsController::generercode('jetoncourrierinternes','JETON',10);
              ...
              {
                $affectation=Affectation::where('courrierinterne_code','=',$courrier_code)
                ->where("uniteadmin_code","=",$uniteadmin_code)
                ->where('uniteadmin_ordonnateur_code', '=', $uniteadmin_ordonnateur_code)
                ->update(["directive_code" =>$directive_code,
                          "priorite" =>$priorite,
          ...
          */
          ...
          if($typeuniteadmin!='DECISIONNEL') 
          {
            if($siTransmission==true)
            {
              ...
              $uniteadmin_recepteur=Courrierinterne::where("code","=",$courrier_code)->pluck("recepteur_code");
              if($uniteadmin_recepteur[0]==$uacode)
              {
                Courrierinterne::where("code","=",$courrier_code)->where("recepteur_code","=",$uacode)->update(["finAffectation" =>1]);
              }
              else
              {
                Affectation::where("courrierinterne_code","=",$courrier_code)->where("uniteadmin_code","=",$uacode)->update(["finAffectation" =>1]);
              }
        ...
        try {
            ...
            //Récupérer l'unité administrative de l'agent connecté
             ...
             /*$query = Courrierinterne::with(['uniteadmin','typecourrier','fichier',
              ...
              $query = Affectation::with(['uniteadmin','directive','affectationscollab','affectationscollab.uniteadmin',
              'courrierinterne',
              'courrierinterne.courrierinitie',
              'courrierinterne.parcours',
              'courrierinterne.etapecourrier',
              'courrierinterne.uniteadmin',
              'courrierinterne.parcours.etapecourrierentrant',
              'courrierinterne.parcours.user',
              'courrierinterne.parcours.user.agent',
              'courrierinterne.noteexplicatives','courrierentrant.noteexplicatives.signatures',
              'courrierinterne.affectations',
              'courrierinterne.affectations.uniteadmin',

            ])
              ->where('uniteadmin_ordonnateur_code','=',$uniteadmin_code)
              ->where('statut','=',1)
              //->where('finAffectation','=',0)
              ->whereHas('courrierinterne', function($q) {
                          $q->where('finAffectation', '=', 1);
                });
    ...
    public function classerinterne(Request $request)
 {
        try { 
            ...
            //Récupérer l'UA de l'acteur connecté
           $uacode=$userconnectdata->agent->uniteadmin->code;
              $uniteadmin_ordonnateur_code=Courrierinterne::where("code","=",$courrier_code)->pluck("recepteur_code");
              if($uacode==$uniteadmin_ordonnateur_code[0]){
                Courrierinterne::where("code","=",$courrier_code)->where("recepteur_code","=",$uacode)->update(["finAffectation" =>1]);
                Affectation::where("courrierinterne_code","=",$courrier_code)->where("uniteadmin_ordonnateur_code","=",$uacode)->update(["statut" =>1]);
