                  ...
                  @foreach($annonces as $an)
                    <div class="col-md-3 col-sm-6">
                        <div class="card card-block">
                          @if($an->image)
                          <img class="img-responsive" src="{{asset('/public/storage/annonces/images/'.$an->image)}}" alt="ManouPicture">
                          @else
                          <img class="img-responsive" src="{{asset('/public/dist-user/images/annonce.png')}}" alt="ManouPicture">
                          @endif
                              <h5 class="card-title mt-3 mb-3">{{ $an->title }}</h5>
                              <p>{{ substr($an->description,0,100) }}...
                              </p>
                              <p class="text-center">
                                <a href="{{route('detailAnnonce',$an)}}" class="btn btn-orange">Lire plus</a>
                              </p>
                        </div>
                      </div>
                    @endforeach
